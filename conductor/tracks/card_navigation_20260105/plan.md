...
## Phase 2: Navigation Logic & UI Refactoring (TDD) [x] [checkpoint: 7e4d70e]
...
- [x] Task: Conductor - User Manual Verification 'Phase 2: Navigation Logic & UI Refactoring (TDD)' (Protocol in workflow.md)

## Phase 3: Visual Feedback & Accessibility [x]
在此階段，我們將加入桌面端的 Hover Tooltips 並優化無障礙支援。

- [x] Task: 撰寫測試，驗證桌面端 Hover 時會顯示正確的 Tooltip 文字 ("Visit Website", "View Code")
- [x] Task: 為主要點擊區域與 GitHub 圖示添加 `aria-label` 以提升無障礙體驗
- [x] Task: 實作 Hover Tooltips（確保在行動端不顯示）
- [x] Task: 進行最後的 linting 與型別檢查 (npm run lint / tsc)
