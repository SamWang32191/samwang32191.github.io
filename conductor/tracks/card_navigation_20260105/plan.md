...
## Phase 1: Preparation & Component Analysis [x]
...
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Preparation & Component Analysis' (Protocol in workflow.md)

## Phase 2: Navigation Logic & UI Refactoring (TDD) [ ]
此階段是核心任務，我們將透過 TDD 模式移除冗餘連結，並實作分區點擊邏輯。

- [ ] Task: 撰寫測試，驗證點擊卡片主要區域（圖片/標題）會觸發前往 Live Site 的導覽
- [ ] Task: 撰寫測試，驗證點擊 GitHub 圖示會觸發前往 Repository 的導覽，且不會觸發 Live Site 導覽
- [ ] Task: 撰寫測試，驗證 "Visit Site" 連結不再出現在組件中
- [ ] Task: 修改 `ProjectCard.tsx` 實作分區點擊邏輯並移除 "Visit Site"
- [ ] Task: 執行測試並修正實作，確保所有測試通過
